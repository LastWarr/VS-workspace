        ...
        # Loop through all the promo amount values 
            for value in range(len(promo_amt)):
                ...
                for inner_index, inner_row in model_input[index:index+len(duration)].iterrows():
                    ...
                    adj_ForecastCY = adj_ForecastCY + adj_Forecast(forecast_CY,p_uplift,1)
                    adj_ForecastNCY = adj_ForecastNCY + adj_Forecast(forecast_NCY,p_uplift,ncypenalty)
                    ...
                    adj_F = adj_ForecastCY + adj_ForecastNCY

                    # Create the row for output dataframe
                    promo_row = pd.DataFrame([[row['Date'],model,duration_flag,promo_amt[value],original_F,adj_F,adj_ForecastCY,adj_ForecastNCY,profit_promo,profit_nopromo]],
                                             columns=['Date','Model','Duration','PromoAmt','Forecast','Adj_Forecast','Adj_ForecastCY','Adj_ForecastNCY','Profit_Promo','Profit_NoPromo'])
...
def promoGraph(data, dataType):
  ...
  colorList = ['gray', 'Greens', 'Reds', 'Blues', 'Portland', 
               'Viridis', 'Earth','Electric', 'mint', 'emrld', #list of colour values for the graph 
               'oranges','teal','purples'  ]
